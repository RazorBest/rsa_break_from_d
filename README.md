...
Which can be written using the quoteient remainder tehorem:
```
...
Which means that a is a square root of 1. This reffers to the known quadratic
...
modulo n are true also modulo p and q. Here is a small proof:
```
Knowing:
...
tells us that given some equalitites modulo some numbers, if the numbers
